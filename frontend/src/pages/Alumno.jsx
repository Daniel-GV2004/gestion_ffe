...
export default function Alumnos() {
  ...
  useEffect(() => {
    fetch('http://127.0.0.1:5000/api/alumno/datos-excel')
      .then((res) => res.json())
      .then((data) => {
        setDatos(data);
        setLoading(false);
  ...
  const rows = datos.map((alumno, index) => (
    <Table.Tr key={index}>
      {Object.values(alumno).map((valor, i) => (
  ...
  return (
    <Container size="xl">
      <Title order={2} mb="md">Listado de Alumnos</Title>
